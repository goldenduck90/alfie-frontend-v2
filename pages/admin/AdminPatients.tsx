...
import Scores from "../../components/pro/ProTable"
...
const adminViewOfPatientsQuery = gql`
  query getAllPatientsByProvider {
    users {
      ...
      score {
        ...
        score30mins
        increased1hour
        percentDifference1Hour
        percentDifference30Mins
        scoreSystolic
        scoreDiastolic
        increasedSystolic
        increasedDiastolic
        percentDifferenceSystolic
        percentDifferenceDiastolic
        providerMessage
      }
      phone
      email
...
export const AdminPatients = () => {
  ...
  const patients = useQuery(adminViewOfPatientsQuery)
  ...
  const [selectedPatient, setSelectedPatient] = useState<Patient>(
    null || patients.data?.users[0]
  )
  console.log(patients, "adminViewOfPatientsQuery")
  const [patientFilterValue, setPatientFilterValue] = useState<string>("")

  useEffect(() => {
    if (selectedPatient?._id) {
      const allPatients = patients.data?.users.find(
      ...
      setSelectedPatient(patients.data?.users[0])
    }
  }, [selectedPatient?._id, patients.data?.users])
  ...
  const filterPatients = patients.data?.users?.filter((patient: Patient) => {
  ...
  return (
    <AdminApplicationLayout title="Patients">
      <SlideOver
      ...
      <div className="flex flex-row h-[84vh]">
        ...
        <div className="flex flex-col w-3/4 pl-8 overflow-y-auto">
          ...
          <div>
            ...
            <div className="pt-10">
              <Scores scores={selectedPatient?.score} />
            </div>
          </div>
          <div className="pt-10">
            <div className="flex flex-col bg-white shadow rounded-lg overflow-hidden">
